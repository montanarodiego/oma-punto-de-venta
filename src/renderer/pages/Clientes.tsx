...
export default function Clientes() {
  ...
  const cargar = useCallback(async (query: string) => {
    setLoading(true);
    try {
      const res = await window.api.clientes.searchPaged({ query: query.trim(), limit: 500 });
      setClientes(res.rows ?? []);
      setTotal(res.total ?? 0);
    } catch (err: any) {
      showToast('Error al cargar clientes: ' + (err.message ?? 'Error.'), 'error');
  ...
  async function verDetalle(c: Cliente) {
    ...
    try {
      const [t, p] = await Promise.all([
        window.api.clientes.getTransacciones(c.id),
        window.api.clientes.listarPagos(c.id),
      ]);
      setTransacciones(t); setPagos(p);
    } catch (err: any) {
      showToast('Error al cargar detalle: ' + (err.message ?? 'Error.'), 'error');
  ...
  const tableHeader = (
    <tr>
      ...
      <th>Teléfono</th>
      <th>Dirección</th>
      <th className="text-right">Límite CC</th>
  ...
  return (
    <div className="page-content">
      ...
      <div className="flex flex-1 overflow-hidden min-h-0">
          ...
          <VirtualTable
            ...
            emptyState={
              <div className="text-center py-12 text-text-subtle text-[13px]">
                ...
                ) : 'Sin clientes. Creá el primero con "+ Nuevo cliente".'}
              </div>
            }
            renderRow={(c, idx) => (
              <tr
                ...
                className={`group cursor-pointer ${clienteDetalle?.id === c.id || tableActiveIdx === idx ? 'bg-[rgba(79,142,245,.1)]' : ''}`}
              >
                ...
                <td className={`text-right font-mono text-[13px] ${c.saldo_vencido > 0 ? 'text-danger font-semibold' : ''}`}>
                  {c.saldo_vencido > 0 ? fmt(c.saldo_vencido) : '—'}
                </td>
                <td onClick={e => e.stopPropagation()}>
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => abrirEditar(c)} className="p-1 text-text-subtle hover:text-accent">
                      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                    </button>
                    <button onClick={() => eliminar(c.id)} className="p-1 text-text-subtle hover:text-danger">
        ...
        {/* Panel de detalle */}
        {clienteDetalle && (
          <div className="w-80 flex-shrink-0 flex flex-col overflow-hidden bg-surface border-l border-border">
            ...
            <div className="p-4 border-b border-border flex flex-col gap-3">
              ...
              <div className="flex gap-2 mt-2">
                {clienteDetalle.saldo_vencido > 0 && (
                  <Button variant="primary" size="sm" className="flex-1" onClick={() => setPagoOpen(true)}>Registrar pago</Button>
                )}
            ...
            <div className="flex-1 overflow-y-auto p-4">
              {detalleLoading ? (
                <div className="text-text-subtle text-[12px] text-center py-4">Cargando…</div>
              ) : (
                <>
                  ...
                  {transacciones.slice(0, 10).map(t => (
                    <div key={t.id} className="flex justify-between items-center py-1.5 border-b border-border-sub last:border-none text-[12px]">
                      ...
                      <span className={`font-mono font-semibold ${t.estado === 'cancelada' ? 'text-danger line-through' : ''}`}>
                        {fmt(t.monto_total)}
                      </span>
                    </div>
                  ))}
                  {transacciones.length === 0 && (
                    <div className="text-text-subtle text-[12px] text-center py-4">Sin transacciones.</div>
                  )}
                </>
              )}
            </div>
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <Button variant="primary" loading={saving} onClick={() => document.getElementById('form-cli')?.dispatchEvent(new Event('submit', { bubbles: true, cancelable: true }))}>Guardar</Button>
          </>
        }
      >
        <form id="form-cli" onSubmit={guardar} className="flex flex-col gap-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Field label="Límite de crédito ($)">
              <Input type="number" step="0.01" min="0" value={form.limite_credito} onChange={e => setForm(p => ({ ...p, limite_credito: parseFloat(e.target.value) || 0 }))} onKeyDown={handleNumericKeyDown}/>
            </Field>
          </div>
      ...
      <Modal
        ...
        footer={
          <>
            <Button variant="ghost" onClick={() => setPagoOpen(false)}>Cancelar</Button>
            <Button variant="primary" onClick={() => document.getElementById('form-pago')?.dispatchEvent(new Event('submit', { bubbles: true, cancelable: true }))}>Registrar</Button>
          </>
        }
      >
        <form id="form-pago" onSubmit={registrarPago} className="flex flex-col gap-4">
          <div className="px-3 py-2 bg-surface-2 rounded-[var(--r-in)] text-[13px]">
            Deuda de <strong>{clienteDetalle?.nombre}</strong>:{' '}
            <span className="text-danger font-mono">{fmt(clienteDetalle?.saldo_vencido ?? 0)}</span>
          </div>
          <Field label="Monto del pago">
            <Input autoFocus type="number" step="0.01" min="0.01" value={pagoMonto} onChange={e => setPagoMonto(e.target.value)} onKeyDown={handleNumericKeyDown} placeholder="0,00" required/>
          </Field>
